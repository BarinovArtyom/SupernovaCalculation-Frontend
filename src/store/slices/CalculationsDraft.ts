...
export const editStar = createAsyncThunk(
  'vacancyApplication/editStar',
  async ({ starId, starData }: { starId: string; starData: any }, { rejectWithValue }) => {
    try {
      const response = await api.star.editUpdate(starId, starData, returnHeaderConfig());
      return response.data;
    } catch (error: any) {
      return rejectWithValue(error.message || 'Ошибка при редактировании заявки');
    }
  }
);
